...
import com.cosmos.vis.BdClient.BdImageIdentify.ImageUnderstand.UnderstandRequest;
import com.cosmos.vis.BdClient.BdImageIdentify.ImageUnderstand.UnderstandResponse;
...
public class ImageUnderstandController {
    ...
    public Result ImageUnderstand(@RequestPart(value = "file", required = false) MultipartFile file,
        ...
        if (question == null || question.isEmpty()) {
            return Result.error("error, question is empty or invalid");
        }
        ...
        if (result == null) {
            return Result.error("很抱歉，图像上传失败");
        }
        ...
        if (retCode == 1) {
            return Result.error("图像正在处理中，请稍后再试");
        }
        String description = resultAns.getString("description");


        return Result.success("success", description);
    }
